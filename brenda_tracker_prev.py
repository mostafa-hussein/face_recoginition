...
class MultiRoomPersonTracker(Node):
    def __init__(self, cam_ports, database_path, display_size=(320, 240), ros_topic='person_location'):
        ...
        self.person_at = "bedroom"  # Initial assumption based on last known location or entry point
        ...
        if time_now - self.last_face_id_time > self.face_recheck_interval:
            ...
            for face in faces:
                ...
                if name == "brian":
                    ...
                    cv2.putText(annotated, f"Brian", (fx1, fy1 - 5),
    ...
    def run_display(self):
        while True:
            # self.check_doors()
            print(f'[INFO] Current person location: {self.person_at} ')
            annotated_frames = {
                room: self.process_frame(room, frame)
                for room, frame in self.frames.items()
            }
            try:
                top = np.hstack((annotated_frames["living_room"], annotated_frames["Doorway"]))
            except Exception as e:
            ...
            for person, info in self.person_locations.items():
                if time.time() - info["last_seen"] < 5:
                    ...
                    cv2.putText(top, status, (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX,1.5, (0, 0, 0), 4)
                    y_offset += 20
            
            grid_small = cv2.resize(top, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA)
            cv2.imshow("Multi-Room Person Tracker", top)
...
def main():
    ...
    try:
        ...
        tracker_node = MultiRoomPersonTracker(
            cam_ports=cam_ports,
            database_path="/home/carl/projects/face_recoginition/face_database_brenda.pkl",
